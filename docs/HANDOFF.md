...
**Poslední aktualizace:** 2026-07-14 (revize dokumentace proti reálnému kódu — sjednoceny modely `bge-m3`/`mistral-nemo`, popsán reconciliation ingest a Entra ID SSO endpointy, srovnány `.env.example`/`requirements.txt`/`axima-web.service`, smazán duplicitní `SPOJERNI_FILE_SERVERU.md`; **doimplementováno OCR obrázkových příloh** v `/api/extract` + 2 drobné opravy kódu)
