...
**Poslední aktualizace:** 2026-08-03 (Optimalizace výkonu a modelů: nasazen špičkový český model `qwen2.5:14b`, odstraněno startovací zpoždění přes `keep_alive: -1`, opravena CI/CD smyčka nastavením passwordless sudo pro restart služby `axima-api` uživatelem `aixima`.)
