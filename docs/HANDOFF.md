...
- **Ověření cest (nové):** tlačítko „Ověřit dostupnost cest" + **terminál**; `POST /api/verify` na serveru provádí dynamic mount SMB share přes `sudo helper` a reálný `os.path.isdir` (s překladem UNC→mount, env `UNC_MOUNT_ROOT`, default `/mnt`). **STAV je do ověření „neověřeno"** — nefabrikuje se (dřív ukazoval fake „dostupná"). Manuální kroky pro nastavení Linuxu jsou v **[docs/LINUX_SETUP_GUIDE.md](LINUX_SETUP_GUIDE.md)**.
...
16. **UNC → mount (deployment TODO):** Operátor zadává `\\server\share\...`, ale Linux backend na to dosáhne jen přes **namountovaný** share (`\\herkules\public\LumirLiduMil` → `/mnt/...`). Podrobný návrh dynamic mountu z webu přes sudo helper a bezpečné credentials je v **[docs/SPOJENI_FILE_SERVERU.md](SPOJENI_FILE_SERVERU.md)**. Manuální kroky na Linux serveru jsou popsány v **[docs/LINUX_SETUP_GUIDE.md](LINUX_SETUP_GUIDE.md)**.
