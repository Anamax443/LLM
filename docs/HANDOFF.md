...
**Poslední aktualizace:** 2026-08-03 (**Manažerský výstup — konec vzorových čísel.** Souhrn i tabulka rozsahu se načítají **živě** z backendu novým `GET /api/scope`: počet dokumentů a bloků z **Qdrantu** (`count` + `scroll` přes `source`), sledované oblasti z **`monitored_paths.json`** (= to samé, co je v Nastavení), časy „aktualizováno" best-effort z **reconciliation manifestu**. Odstraněna natvrdo psaná data „128 / 2 940 / 4 / 1. 7. 2026" i smyšlené cesty `\\fileserver\Smernice…`. Co nelze zjistit (typicky čas bez manifestu) → **„—", nefabrikuje se**. Sloupec „Kategorie" přejmenován na **„Přístup"** a ukazuje reálné ACL z Nastavení (Veřejné / Omezené + uživatelé/skupiny). Dokumenty mimo aktuálně sledované cesty se ukážou jako řádek „(mimo sledované cesty)".)
**Předchozí:** 2026-08-03 (UI: hlavička (logo AXIMA + název) je nově **klikací → přejde na homepage / záložku Asistent** — přes existující mechanismus `data-tab-link`, hover + focus-visible stav.)

---

## 1. Co je hotové

- **Backend (POC, funkční):** `watchdog_service.py` (reconciliation ingest), `api.py` (`POST /ask` + auth/verify/extract/monitored_paths), `ask_ai.py` (CLI). Stack Ollama (`bge-m3` 1024D embedding + **`qwen2.5:14b`** generování) + Qdrant (`axima_docs`, 1024D). Načítání `.env`, integrace **Microsoft Entra ID (SSO)** pro autentizaci, podpora DOCX a dočasných příloh. Model natrvalo načtený v GPU (`keep_alive: -1`) pro okamžité reakce. *(Pozn.: inline komentář ve `watchdog_service.py` u vytvoření kolekce ještě zmiňuje „768 / nomic-embed-text", ale kód vytváří kolekci reálně na 1024D pro `bge-m3` — komentář je zavádějící, k opravě v kódu.)*
- **Web UI (nové):** `web/index.html` — homepage dle AXIMA UI standardu, **AXIMA logo v hlavičce** (**klikací → přejde na homepage / záložku Asistent**), 4 záložky, dark+light, tisk light (**logo v hlavičce tisku**), CS+EN, servisní řádek **v hlavičce** (health + model + commit + hodiny + GitHub). **Nově:** **streamovaný chat** s historií konverzace a podporou Markdownu, tlačítko "Zastavit" generování. Nastavení má **Stav skenování** (poslední/příští sken, „Skenovat teď", per-cesta dokumenty/bloky/čerstvost). Dokumentace = **plnohodnotné firemní články uvnitř aplikace** (boční menu, CS+EN, bez odkazů na git/soubory). Nyní **komunikuje s backendem** pro načítání a ukládání monitorovaných cest. Nově spouštěno jako `axima-web.service` přes systemd. **UX vylepšení:** Přepracované popisky konfigurace a nápovědy pro laiky (ikony otazníku s hover textem), opraven layout formuláře "Model a hledání" pro symetrii a zarovnání tooltipů, opravená lokalizace (i18n) klíčů (odebrány emoji) a nahrazen nestandardní atribut `help` za `title`.
- **Přílohy k dotazu (nové):** v Asistentu tlačítko 📎 + **vkládání ze schránky (Ctrl+V screenshot)** + náhled příloh; obsah se přes `POST /api/extract` převede na text a přidá ke kontextu. **Reálně implementované parsery:** PDF (`pypdf`), DOCX (`python-docx`), **obrázky/screenshoty přes OCR** (`pytesseract`, `ces+eng`, graceful fallback když chybí tesseract/jazyková data), textový fallback (UTF-8). Deploy pro upload + OCR: `pip install python-multipart pytesseract Pillow` + systémový `tesseract-ocr` + `tesseract-ocr-ces`. **OCR skenovaných PDF a tabulek = stále roadmapa** (krok „Parsing"). Opravena striktní RAG blokáda u doplňujících otázek k dříve nahraným přílohám.
...
**Opraveno oproti starším poznámkám (už NENÍ bug):** stale vektory při update (reconciliation maže staré bloky zdroje před upsertem), `on_deleted` (bloky smazaných souborů se odstraní), normalizace lomítek (falešná detekce smazání), CORS pre-flight, iterace historie v `/ask`, mrtvý duplicitní blok `messages` v `/ask` (odstraněn), zavádějící komentář 768/nomic ve `watchdog_service.py` (opraven na 1024/bge-m3), **OCR obrázkových příloh** (implementováno v `/api/extract`, `pytesseract` `ces+eng`), **zpoždění modelu** (odstraněno pomocí `keep_alive: -1`), **chyba v CI/CD** (opraveno přes `/etc/sudoers.d/aixima_cicd` pro `systemctl restart axima-api` bez hesla).
