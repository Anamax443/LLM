...
**Poslední aktualizace:** 2026-07-02 (UI: streaming odpovědí, chat historie, Markdown, Stop tlačítko)
...
- **Ověření cest (nové):** tlačítko „Ověřit dostupnost cest" + **terminál**; `POST /api/verify` na serveru dělá reálný `os.path.isdir` (s překladem UNC→mount, env `UNC_MOUNT_ROOT`, default `/mnt`). **STAV je do ověření „neověřeno"** — nefabrikuje se (dřív ukazoval fake „dostupná").
...
16. **UNC → mount (deployment TODO).** Operátor zadává `\\server\share\...`, ale Linux backend na to dosáhne jen přes **namountovaný** share (`\\herkules\public\LumirLiduMil` → `/mnt/...`). Bez toho `/api/verify` (os.path.isdir) vždy vrátí „nedostupná". Řešení při nasazení: mountovat sledované share, nebo přidat do backendu mapu UNC→mount. Zatím vědomě NEstavíme (premature před deployem).
