...
- **Přílohy k dotazu:** endpoint `POST /api/extract` převede přiložené soubory na text (PDF přes `pypdf`, DOCX přes `python-docx`, textový fallback). **OCR obrázků zatím není implementováno** — je v roadmapě (viz [HANDOFF.md](docs/HANDOFF.md) / [OPONENTURA.md](docs/OPONENTURA.md) krok „Parsing").
- **AI Backend:** Databáze Qdrant (port 6333, kolekce `axima_docs`, vektor 1024D, kosinová vzdálenost). Modely běží přes Ollamu (port 11434). Model `bge-m3` pro vektorizaci textu (1024D) a `mistral-nemo` pro generování odpovědí (temperature 0.0).
